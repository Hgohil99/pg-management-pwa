...
async function loadCleaningPageData() {
  ...
  if (todayDoc.exists) {
    ...
    todayEl.innerHTML = '<p>No assignment yet</p>';
  }
  ...
  if (tomorrowDoc.exists) {
    ...
    tomorrowEl.innerHTML = '<p>Not assigned yet</p>';
  }
}
...
async function confirmCleaning(dateStr) {
  try {
    await db.collection('cleaningAssignments').doc(dateStr).update({
...
// Auto-assign from present list
async function autoAssignCleaning() {
  const users = await getAllUsers();
  const presentUsers = users.filter(u => u.present).sort((a, b) => (a.order || 0) - (b.order || 0));
  
  if (presentUsers.length === 0) {
    alert('No one is present!');
    return;
  }

  const lastAssignment = await db.collection('cleaningAssignments')
  ...
  if (!lastAssignment.empty) {
    ...
    if (lastIndex >= 0) {
      nextIndex = (lastIndex + 1) % presentUsers.length;
    }
  }
  ...
  await db.collection('cleaningAssignments').doc(tomorrow()).set({
    ...
    finalAssigneeName: user.name,
    confirmedAt: null,
    assignedAt: firebase.firestore.FieldValue.serverTimestamp()
  });
  
  await sendNotification(user.id, 'Cleaning Duty', `You are assigned for kitchen cleaning tomorrow (${formatDisplayDate(tomorrow())}). Please confirm.`, '/');
  
  alert('✅ Auto-assigned to ' + user.name + '!');
  loadCleaningPageData();
}

// Assign cleaning manually
async function assignCleaningManually() {
  const users = await getAllUsers();
  const presentUsers = users.filter(u => u.present).sort((a, b) => (a.order || 0) - (b.order || 0));
  
  if (presentUsers.length === 0) {
    alert('No one is present tomorrow!');
    return;
  }
